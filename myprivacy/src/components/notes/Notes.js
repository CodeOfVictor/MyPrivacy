...
function Notes() {
  ...
  const notes = [
    { title: 'Title 1', date: '01/01/2021', description: 'Description 1' },
    { title: 'Title 2', date: '02/02/2022', description: 'Description 2' },
    { title: 'Title 3', date: '03/03/2023', description: 'Description 3' },
  ];
    ...
    <Grid container>
      ...
      <Grid container style={{ marginLeft: '2%', marginRight: '2%' }}>
        {notes.map((note, index) => (
          <Note key={index} title={note.title} date={note.date} description={note.description} />
        ))}
      </Grid>
    </Grid>
  );
